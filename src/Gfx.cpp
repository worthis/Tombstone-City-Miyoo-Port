...
void GfxInit()
{
   ...
   */
#ifdef PSP
  render = SDL_SetVideoMode(GFX_RENDER_W, GFX_RENDER_H, GFX_SCREEN_DEPTH,
                            SDL_ANYFORMAT | SDL_DOUBLEBUF | SDL_HWSURFACE | SDL_HWPALETTE);
#elif defined(MIYOO_MINI)
  render = SDL_SetVideoMode(GFX_RENDER_W, GFX_RENDER_H, GFX_SCREEN_DEPTH, SDL_HWSURFACE | SDL_DOUBLEBUF);
#else
  render = SDL_SetVideoMode(GFX_RENDER_W, GFX_RENDER_H, GFX_SCREEN_DEPTH, SDL_SWSURFACE);
#endif
  if (render == NULL)
  {
    fprintf(stderr, "Couldn't set %dx%dx%d video mode: %s\n", GFX_RENDER_W, GFX_RENDER_H, GFX_SCREEN_DEPTH, SDL_GetError());
    exit(1);
  }

  screen = SDL_CreateRGBSurface(SDL_HWSURFACE, GFX_SCREEN_W, GFX_SCREEN_H,
                                render->format->BitsPerPixel, render->format->Rmask, render->format->Gmask, render->format->Bmask, 0);

#ifdef MIYOO_MINI
  buffer = SDL_CreateRGBSurface(SDL_HWSURFACE, GFX_RENDER_W, GFX_RENDER_H,
                                render->format->BitsPerPixel, render->format->Rmask, render->format->Gmask, render->format->Bmask, 0);
#endif
...
bool GfxInitWH(UInt16 w, UInt16 h)
{
  if (screen)
  {
    fprintf(stderr, "Screen already initialized\n");
    return false;
  }
  ...
  if (!screen)
  {
    fprintf(stderr, "Couldn't set %dx%dx%d video mode: %s\n", w, h, GFX_SCREEN_DEPTH, SDL_GetError());
    return false;
  }
...
void GfxChar(UInt8 ch, UInt32 fgColor, UInt32 bgColor)
{
  ...
  for (iline = 0; iline < FONT_HEIGHT; iline++)
  {
    if (font8x8 == 0)
      ...
      line = font8x8[(ch * FONT_WIDTH) + iline];
...
void GfxCharXY(gfxbitmap *bitmap, UInt8 ch, UInt16 x, UInt16 y, UInt32 fgColor, UInt32 bgColor)
{
  ...
  for (iline = 0; iline < FONT_HEIGHT; iline++)
  {
    ...
    for (i = 0; i < FONT_WIDTH; i++)
    {
      ...
      if (data)
        color = fgColor; // ink color (foreground)
      else
        color = bgColor; // paper color (background)
...
gfxbitmap *GfxCreateBitmapFromFile(const char *imagefile)
{
  if (!imagefile)
    return 0;
...
#ifdef GIF_SUPPORT
  string fileName = string(imagefile) + string(".gif");
  bitmap = GfxCreateBitmapFromGif(fileName.c_str());
#else
  string fileName = string(imagefile) + string(".bmp");
...
gfxbitmap *GfxCreateBitmapFromGif(const char *giffile)
{
  ...
  if ((GIFfile = DGifOpenFileName(giffile)) == 0)
  {
    return 0; // Error
  }
...
void GfxUpdate()
{
  ...
  SDL_BlitSurface(buffer, NULL, render, NULL);
  SDL_Flip(render);
#else
