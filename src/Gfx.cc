...
void GfxInit () {
 ...
 if( SDL_Init(SDL_INIT_VIDEO) < 0 ) {
   ...
   */
# ifdef PSP
  screen = SDL_SetVideoMode(GFX_SCREEN_W, GFX_SCREEN_H, GFX_SCREEN_DEPTH, 
                                SDL_ANYFORMAT|SDL_DOUBLEBUF|SDL_HWSURFACE|SDL_HWPALETTE);
# else
  screen = SDL_SetVideoMode(GFX_SCREEN_W, GFX_SCREEN_H, GFX_SCREEN_DEPTH, SDL_SWSURFACE);
# endif
  if ( screen == NULL ) {
    fprintf(stderr, "Couldn't set %dx%dx%d video mode: %s\n",GFX_SCREEN_W, GFX_SCREEN_H, GFX_SCREEN_DEPTH, SDL_GetError());
    exit(1);
  }
  GfxSetNoClip (0);
